...
5. “我问完了，整理入库”“这题到这里，归档吧”“ok了，整理一下这道题目的思路”“好了，整理一下这道题目的思路”“行了，归纳一下本题”都是 archive。“ok/ok了”“好了”“行了”等表达在这些语境中明确表示本轮结束，后面的总结或整理是归档要求；没有任何明确结束表达时，同样的总结或整理请求仍是 question。
...
_EXPLICIT_FINISH_CUES = re.compile(
    ...
    r"(?:ok|OK|好了|行了).{0,8}(?:总结|整理|归纳|梳理|归档|入库))"
)
...
_OK_THEN_ARCHIVE_CUE = re.compile(
    r"(?:^|[\s，。！？、,.!?：:；;])(?:ok|好了|行了).{0,12}(?:总结|整理|归纳|梳理|归档|入库|收录)",
    re.I,
)
...
_HYPOTHETICAL_FINISH_CUES = re.compile(
    ...
    r"(?:ok|好了|行了).{0,12}(?:总结|整理|归纳|梳理|归档|入库|收录))",
    re.I,
)
...
class MessageClassifier:
    ...
    def _is_finish_only_control(text: str) -> bool:
        ...
        return bool(
            re.fullmatch(
                r"(?i)(?:ok了|ok|好了|行了)(?:请|帮我)?(?:总结|整理|归纳|梳理|归档|入库|收录)"
                r"(?:一下)?(?:(?:这(?:道)?题目?|本题)(?:的)?(?:题干|思路|解法|方法|题型|知识点|做法)?)?(?:吧|了)?",
                normalized,
            )
        )
