...
AstrBot 私聊考研答疑归档插件。它不会接管或修改 AstrBot 的正常回答，只在精确配置的私聊 UMO 中旁路保存消息。每条自然语言消息由 LLM 判断为“问题、归档边界、取消边界、其他软指令”，遇到归档边界后把上一边界到当前边界之间的有效对话整理为一道题目。
...
- 自然语言分类为 `question`、`archive`、`cancel` 或 `instruction`；软指令保存但不进入题目正文。分类模型链路全部失效时，事件进入待分类队列，同样不会进入正文。
