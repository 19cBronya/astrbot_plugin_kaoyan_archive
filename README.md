...
- 全部消息页支持原文搜索、会话/类型/归属筛选、查看消息与题目的全部关系，并可批量改归属、设为未归档或归档为一道新题目；操作自动扩展到完整问答轮次。
...
“全部消息”页面展示数据库中的每条不可变原始事件及其所有题目关系，包括正文、补充、回答、边界和排除审计。可把同一 UMO 的普通消息归入已有题目，也可把已经归档的消息重新设为未归档。选择用户消息时会同时处理它的全部助手回复；选择助手回复时会反向扩展到所属用户消息及同轮回复，防止产生孤立回答。结束边界和框架指令只读，不允许从页面破坏区间语义。
...
软删除题目的详情页也提供“归档为新题目”。该操作把删除记录当前拥有的有效问答轮次移入新的内部 UUID，并在整理成功时分配新题号；旧题继续保持软删除，旧关系转为排除审计。后台会追加一条明确标记为 `dashboard_manual_archive` 的只读管理边界，使新题来源可审计而无需伪造聊天原文；该管理边界不会改变后续自然聊天的区间起点。
